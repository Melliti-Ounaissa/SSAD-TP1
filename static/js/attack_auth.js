...
// attack_auth.js - Handle password attack UI and backend calls
...
async function startAttack() {
  ...
  try {
    ...
    } else if (selectedMethod === "bruteforce5") {
      addLogEntry("💥 Mode: Force brute (5 caractères: a-z, 0-9)", "trying")
    } else if (selectedMethod === "bruteforce6") {
    ...
    if (attackData.success) {
      ...
      showError(attackData.message || "Attack failed")
      isAttackRunning = false
      startAttackBtn.disabled = false
    }
  } catch (error) {
...
// Display the final result from the server
